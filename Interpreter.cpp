...
{
    FarmScript::runtimeError(oper, message);
}
...
{
    Object value = evaluate(stmt->expression);
    if (repl && !FarmScript::hadRuntimeError)
    {
        std::cout << value.str() << std::endl;
    }
}
...
Object Interpreter::executeBlock(Block *stmt, Environment *execEnvironment)
{
    ...
    {
        execute(s);
        if (breakSet || returnSet || FarmScript::hadRuntimeError)
        {
            break;
        }
    }
...
void Interpreter::visitClass(Class *stmt)
{
    ...
    if (stmt->superclass)
    {
        ...
        {
            FarmScript::runtimeError(*stmt->superclass->name, "Superclass must be a class.");
            return;
        }
