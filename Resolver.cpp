...
void Resolver::endScope()
{
    ...
    for (const auto &pair : *scope)
    {
        ...
        {
            //FarmScript::error(0, "Unreferenced variable: " + pair.first);
        }
    }
    delete scope;
    scopes.pop_back();
...
void Resolver::declare(Token * name)
{
    ...
    if (it != scope->end())
    {
        FarmScript::error(*name, "Variable already defined in this scope.");
    }
...
void Resolver::visitVariable(Variable * expr)
{
    if (!scopes.empty())
    {
        ...
        if (it != scope->end())
        {
            ...
            {
                FarmScript::error(*expr->name, "Can't read local variable in its own initializer.");
            }
...
void Resolver::visitSuper(Super *expr)
{
    ...
    {
        FarmScript::error(*expr->keyword, "Can't use 'super' outside of a class.");
        return;
    }
    else if (currentClass != ClassType_Subclass)
    {
        FarmScript::error(*expr->keyword, "Can't use 'super' in a class with no superclass.");
        return;
    }
...
void Resolver::visitThis(This *expr)
{
    if (currentClass == ClassType_None)
    {
        FarmScript::error(*expr->keyword, "Can't use 'this' outside of a class.");
        return;
    }
    resolveLocal(expr, expr->keyword);
}
...
void Resolver::resolve(Expr * expr)
{
    if (expr)
    {
        switch (expr->type)
        {
            ...
            default: FarmScript::error(0, "Invalid expression type.");
        }
    }
}
...
void Resolver::visitClass(Class *stmt)
{
    ...
    if (stmt->superclass)
    {
        ...
        {
            FarmScript::error(*stmt->superclass->name, "A class can't inherit from itself.");
        }
    ...
    for (auto method : stmt->statics->list)
    {
        ...
        if (method->name->lexeme == "init")
        {
            FarmScript::error(0, "'init' cannot be a static method");
        }
        resolveFunction(method, declaration);
    }
...
void Resolver::resolveFunction(void *stmt, FunctionType type)
{
    ...
    {
        FarmScript::error(0, "FunctionType not implemented!");
    }
...
void Resolver::visitReturn(Return * stmt)
{
    ...
    {
        FarmScript::error(*stmt->keyword, "Can't return from top-level code.");
    }
    ...
    if (stmt->value)
    {
        ...
        {
            FarmScript::error(*stmt->keyword, "Can't return a value from an initializer.");
        }
